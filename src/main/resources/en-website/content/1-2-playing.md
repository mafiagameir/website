...
You can start the game with the [start command](#start). Remember that you need to enter the necessary parameters along with the start command (the number of citizens, number of Mafia, number of detectors, and number of doctors).
...
After the user is killed by Mafia it's the detectors’ turn. The bot will privately ask the detector whom he/she suspects, and then the detector enter the suspect's name using the [ask command](#ask). The bot will then let him know if the suspect is Mafia or not.


After mafia and detector, it is now the Doctor’s turn. Doctor can heal one player per night. The Bot will privately ask the doctor to name someone to rescue. Doctor has to rescue a player by [heal command](#heal).
